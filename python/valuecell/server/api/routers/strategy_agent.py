...
def create_strategy_agent_router() -> APIRouter:
    ...
    async def create_strategy_agent(
        ...
        try:
            ...
            # Keep exchange_id in virtual mode: market data (candles, snapshot) is
            # fetched from this exchange; only execution uses the paper gateway.
            if (
                request.exchange_config.trading_mode == TradingMode.VIRTUAL
                and request.exchange_config.exchange_id in {None, ""}
            ):
                request.exchange_config.exchange_id = "binance"
                logger.info(
                    "Virtual trading: no exchange_id set; defaulting market data to binance"
                )
    ...
    async def test_exchange_connection(request: ExchangeConfig):
        ...
        try:
            ...
            try:
                ...
                if is_connected:
                    return SuccessResponse.create(msg=message)
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    "Connection failed. Please check your API Key, Secret Key, "
                    "passphrase (if required), and IP whitelist on the exchange."
                ),
            ) from exc
