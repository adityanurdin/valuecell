...
class LlmComposer(BaseComposer):
    ...
    def _build_summary(self, context: ComposeContext) -> Dict:
        ...
        session_trade_count = sum(
            int(getattr(entry, "trade_count", 0) or 0)
            for entry in (digest.by_instrument or {}).values()
        )
        ...
        # Equity micro-noise produces extreme Sharpe values before any trade.
        # Omit sharpe until there is execution history so the LLM can follow
        # strategy prompts that gate on trade_count instead of a bogus halt.
        if session_trade_count > 0 or active_positions > 0:
            summary["sharpe_ratio"] = digest.sharpe_ratio
        else:
            summary["sharpe_ratio_note"] = (
                "Sharpe omitted: no executed trades yet. "
                "Do not apply Sharpe halt; evaluate entry signals only."
            )
